...
Page({
    ...
    page_list: [
      ...
      },
      {
        ...
        list: [
          ...
          {
            name: "卡片叠层百叶窗",
            url: "/pages/subPack/card_window/index",
          },
          {
            name: "自定义input输入框",
  ...
  so_input:tool.debounce(function(e) {
    let that = this;
    let value = e[0].detail.value;
    let page_list = that.data.page_list;
    let page_data=[];
    for(let i=0;i<=page_list.length-1;i++){
      let show_list={};
      show_list.showmore=page_list[i].showmore;
      show_list.name=page_list[i].name;
      show_list.list=[];
      for(let j=0;j<=page_list[i].list.length-1;j++){
        if(page_list[i].list[j].name.indexOf(value)!=-1){
          show_list.list.push(page_list[i].list[j])
        }
      }
      if(show_list.list.length>0){
        page_data.push(show_list)
      }
    } 
    that.setData({
      page_show:page_data
